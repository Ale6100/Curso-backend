...
const checkLogger = (req, res, next) => { // Se encarga de obtener la información del usuario actual, si es que está logueado
    ...
    try {
        const user = jwt.verify(token, config.jwt.secret); // Descifra al usuario siempre y cuando el token de la cookie no haya vencido (da error si venció o si no se pasó ningún token)
